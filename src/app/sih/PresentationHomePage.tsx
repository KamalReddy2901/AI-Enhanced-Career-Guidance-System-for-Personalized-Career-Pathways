...
const core = [
  ...
  ['06', 'Recruiter', 'Recruiter review', 'Review consented evidence. Private Career Guidance remains private.', '/industry/applicants/a080bafe-ec71-4fd5-99b2-19ed8ac8cb87'],
...
export function PresentationHomePage() {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8 md:py-10">
    ...
    <section className="mt-8" aria-labelledby="core-story"><div className="flex items-baseline justify-between gap-4"><h2 id="core-story" className="font-display text-3xl">Core story</h2><span className="font-mono-ui text-[10px] font-black uppercase tracking-wide">7 steps</span></div><ol className="mt-4 grid gap-3 md:grid-cols-2">{story.map(([number, persona, title, detail, to]) => <li key={number} className="grid grid-cols-[auto_1fr] gap-x-4 border-2 border-black bg-white p-4 shadow-[3px_3px_0_#111]"><span className="font-mono-ui text-xl font-black text-[var(--accent-news)]">{number}</span><div><p className="font-mono-ui text-[10px] font-black uppercase tracking-wide text-black/50">{persona}</p><h3 className="mt-1 text-xl font-black">{title}</h3><p className="mt-1 text-sm leading-5 text-black/65">{detail}</p>{number === '06' || number === '07' ? <button type="button" onClick={() => openStep(number, to)} className="mt-3 inline-flex min-h-9 items-center border-2 border-black bg-[#e7ff57] px-3 font-mono-ui text-[10px] font-black uppercase">Open</button> : <Link to={to} className="mt-3 inline-flex min-h-9 items-center border-2 border-black bg-[#e7ff57] px-3 font-mono-ui text-[10px] font-black uppercase">{(number === '02' || number === '03') && !flagship ? 'Loading' : 'Open'}</Link>}</div></li>)}</ol></section>
