...
export function StopPress({ changes, onDismiss }: { changes: RecommendationChange[]; onDismiss: () => void }) {
  ...
  return <aside className="relative mb-8 rotate-[-.35deg] border-y-4 border-double border-black bg-white p-5 shadow-[4px_4px_0_rgba(0,0,0,.08)]" aria-live="polite">
    ...
    <ul className="mt-3 space-y-2 pr-10">{changes.map(change => <li key={change.occupationId} className="font-[Inter] text-sm">
      <strong>{occupationById.get(change.occupationId)?.title ?? change.occupationId}</strong> {change.previousScore}→{change.score} · {change.rank < change.previousRank ? `▲ ${change.previousRank - change.rank}` : change.rank > change.previousRank ? `▼ ${change.rank - change.previousRank}` : 'rank unchanged'}
    </li>)}</ul>
