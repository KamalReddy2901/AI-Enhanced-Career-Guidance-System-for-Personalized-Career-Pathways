...
export function GapClosurePlan({ result, opportunityVersion, availability, errorMessage, onEvidenceHandoff }: Props) {
  ...
  return <section className="grid gap-6" aria-labelledby="gap-closure-plan-title">
    ...
    {actions.length === 0 ? <div className="border-2 border-dashed border-black bg-white p-5"><h2 className="text-xl font-black">No closure actions are currently indicated.</h2><p className="mt-2 text-sm text-black/70">Current requirements are either strongly supported or not applicable.</p></div> : order.map(group => {
      ...
      return <section key={group} aria-labelledby={`${group}-actions`}><h2 id={`${group}-actions`} className="text-2xl font-black">{headings[group]}</h2><div className="mt-3 grid gap-3">{items.map(action => <article key={action.id} className="border-2 border-black bg-[#f7f4ed] p-4"><div className="flex flex-wrap justify-between gap-3"><h3 className="font-bold leading-relaxed">{action.title}</h3><span className="border border-black bg-white px-2 py-1 font-mono-ui text-[10px] font-bold uppercase">{label(action.kind)}</span></div><p className="mt-3 text-sm leading-relaxed">{action.reason}</p><p className="mt-3 text-xs text-black/65"><span className="font-bold">Expected evidence or result:</span> {action.expectedEvidence}</p>{action.kind === 'PROVE_EXISTING' && onEvidenceHandoff && <button type="button" onClick={() => onEvidenceHandoff({ opportunityId: result.opportunityId, opportunityVersionId: result.opportunityVersionId, readinessResultId: result.resultId, requirementId: action.requirementId, action })} className="mt-4 min-h-11 border-2 border-black bg-[#e7ff57] px-4 font-mono-ui text-xs font-black uppercase focus-visible:outline focus-visible:outline-4 focus-visible:outline-[#ff5c35]">Continue to evidence</button>}</article>)}</div></section>;
    })}
  </section>;
}
