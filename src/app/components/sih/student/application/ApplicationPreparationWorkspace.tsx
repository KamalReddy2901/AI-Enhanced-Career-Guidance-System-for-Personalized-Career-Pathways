...
type Props = {
  ...
  readonly onSelectedEvidenceRecordIdsChange: (ids: readonly string[]) => void;
...
export function ApplicationPreparationWorkspace({ evidence, selectedEvidenceRecordIds, onSelectedEvidenceRecordIdsChange, disclosure, questionnaireReference }: Props) {
  ...
  function toggle(id: string) { onSelectedEvidenceRecordIdsChange(selected.has(id) ? selectedEvidenceRecordIds.filter(existing => existing !== id) : [...selectedEvidenceRecordIds, id]); }
  return <section className="grid gap-6" aria-labelledby="application-preparation-title">
    ...
    <section className="border-2 border-black bg-[#f7f4ed] p-5" aria-labelledby="supporting-evidence-title"><h2 id="supporting-evidence-title" className="text-xl font-black">Supporting evidence</h2>{evidence.length === 0 ? <p className="mt-3 text-sm text-black/70">No authorized evidence records are available to select.</p> : <div className="mt-4 grid gap-3">{evidence.map(record => <label key={record.id} className="flex cursor-pointer gap-3 border-2 border-black bg-white p-4"><input type="checkbox" checked={selected.has(record.id)} onChange={() => toggle(record.id)} className="mt-1 h-5 w-5 accent-black" /><span><span className="block font-bold">{record.literalClaim}</span><span className="mt-1 block font-mono-ui text-[10px] uppercase text-black/60">{record.provenance.replaceAll('_', ' ')} · {record.visibility.replaceAll('_', ' ')}</span></span></label>)}</div>}</section>
