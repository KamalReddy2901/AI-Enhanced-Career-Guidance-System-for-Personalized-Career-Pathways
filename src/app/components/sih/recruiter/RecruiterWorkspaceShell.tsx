          ...
          {selectedApplication ? (
            <RecruiterAccessState state={projectionAccessState}>
              {projectionAccessState === 'available' && projection && (
                <>
                  <aside className="border-l-4 border-black bg-[#fff4c7] p-4" aria-label="Recruiter privacy boundary"><p className="font-mono-ui text-[10px] font-black uppercase tracking-wide">Private career guidance is not shared with recruiters</p><p className="mt-1 text-sm text-black/70">Recruiters see only the application snapshot, opportunity-specific readiness, consented evidence, relevant artifacts and application history. Personal guidance profiles and counselor context remain private.</p></aside>
                  <ApplicationDetailView projection={projection} opportunityTitle={opportunityTitle} presentationApplicantName={selectedApplication.id === 'a080bafe-ec71-4fd5-99b2-19ed8ac8cb87' ? 'Ananya Rao' : undefined} currentStage={selectedApplication.currentStage} />
