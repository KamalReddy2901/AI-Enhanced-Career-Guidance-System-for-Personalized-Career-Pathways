...
export function ApplicationRecruitmentTimeline({
  ...
  const presentationMessage = (record: ApplicationRecruitmentRecordReadModel) =>
    isPresentationMode() && record.visibility === 'recruiter_internal' && /controlled|fixture|synthetic|acceptance/i.test(record.message ?? '')
      ? 'Application review completed against consented evidence and the recorded readiness snapshot.'
      : record.message;
  return (
        ...
        <ol className="mt-5 grid gap-3">
          {records.map(record => (
            <li key={record.id} className={`border p-4 ${record.visibility === 'recruiter_internal' ? 'border-[#d63c1d] bg-[#fff1ec]' : 'border-black/20 bg-[#f7f4ed]'}`}>
              ...
              {presentationMessage(record) && <p className="mt-2 whitespace-pre-wrap text-sm leading-6">{presentationMessage(record)}</p>}
