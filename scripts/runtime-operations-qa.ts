...
const [connectors, connectorFramework, notifications, notificationMigration, notificationHardening, worker, sihI18n] = await Promise.all([
  ...
  readFile(join(root, 'supabase/migrations/20260831120000_notification_claim_and_questionnaire_function_hardening.sql'), 'utf8'),
  readFile(join(root, 'worker/src/index.ts'), 'utf8'),
  readFile(join(root, 'src/app/i18n/sih.ts'), 'utf8'),
]);
for (const term of ['ConnectorRegistry', 'ConnectorPage', 'NormalizedConnectorRecord', 'ConnectorWebhookEvent', 'minimizeAtsExportPayload', 'ATS_EXPORT_ALLOWED_FIELDS', 'dedupeConnectorRecords', 'reconcileConnectorRecords', 'connectorRetryDelayMs']) assert.match(connectors, new RegExp(term));
for (const term of ['authRequirement', 'syncDirection', 'ConnectorRuntime', 'handleWebhook', 'configuration', 'audit', 'INTEGRATION-READY', 'createDefaultConnectorRegistry']) assert.match(connectorFramework, new RegExp(term));
...
assert.match(notificationHardening, /revoke execute on function sih26044\.claim_notification_outbox\(integer, integer\)[\s\S]*authenticated/i);
assert.match(notificationHardening, /block_published_questionnaire_mutation\(\)[\s\S]*set search_path = pg_catalog, sih26044/i);
