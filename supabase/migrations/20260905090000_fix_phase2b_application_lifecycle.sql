-- Repair: Fix historical application lifecycle in phase-2B seed.
--
-- Two defects found in production execution of PR #88:
--
-- Defect 1: preparing→applied event missing application_snapshot_id column.
--   application_events has a CHECK constraint:
--     (to_stage = 'applied' AND application_snapshot_id IS NOT NULL)
--   and the updated enforce_application_event_sequence() trigger validates the
--   snapshot is finalized and bound to this application+opportunity_version.
--
-- Defect 2: applied→active is not a valid transition.
--   application_transition_allowed() allows:
...
--
-- This migration replaces seed_controlled_demo_ecosystem_phase2b() with:
--   1. application_snapshot_id set on the preparing→applied event
--   2. Corrected full lifecycle: preparing→applied→under_review→offered→accepted→active→completed→outcome_recorded
--   3. Idempotent event insertion via current_application_stage() guards
--   4. All PR #87 fixes preserved (questionnaire conflict key, version_id in assignments, no intervention events)
...
begin
  ...
  insert into sih26044.opportunity_versions (
    ...
    (v_ver_backend_dev,     v_opp_backend_dev,     1, 'draft', 'Backend Developer Intern',               'Build REST APIs for Meridian Analytics Labs data platform. Duration: 6 months | Remote | ₹22,000/month',                                                                  'internship',          ARRAY['student']::sih26044.opportunity_audience[],         'controlled_fixture', 'Controlled synthetic demo opportunity', '2026-08-25'::timestamptz, v_actor_recruiter_b, '2026-08-25'::timestamptz, null),
    ...
    (v_ver_live_project,    v_opp_live_project,    1, 'draft', 'Live Industry Data Challenge – Health Analytics', 'A 4-week industry live project at Meridian Analytics Labs. Teams analyse a real anonymised health dataset and present to industry judges.',                        'live_project',        ARRAY['student']::sih26044.opportunity_audience[],         'controlled_fixture', 'Controlled synthetic demo opportunity', '2026-08-27'::timestamptz, v_actor_recruiter_b, '2026-08-27'::timestamptz, null),
  ...
  -- Publish all background opportunity versions
  ...
  insert into sih26044.questionnaire_versions (
    ...
    (v_qv_data_analyst, v_q_data_analyst, 1, 'draft', 'Data Analyst Technical Screening', 'Assesses core SQL, Python, and analytical reasoning skills for data analyst intern roles.', 'reusable_technical', '{"version": "1.0", "rules": [{"questionOrdinal": 0, "maxScore": 4}, {"questionOrdinal": 1, "maxScore": 4}, {"questionOrdinal": 2, "maxScore": 4}]}'::jsonb, v_actor_recruiter_b, '2026-08-20'::timestamptz, null),
  ...
  -- HISTORICAL COMPLETED APPLICATION: Arjun → Live Industry Data Challenge
  --
  -- Lifecycle: preparing → applied → under_review → offered → accepted
  --            → active → completed → outcome_recorded
  --
  -- Fix 1: preparing→applied event includes application_snapshot_id = v_snap_hist
  --   (required by application_events CHECK constraint and trigger)
  -- Fix 2: corrected transition sequence — applied→under_review→offered→accepted→active
  --   (applied→active is not a valid transition)
  -- Fix 3: idempotent — each event only appended if current_application_stage
  --   matches the expected from_stage
  -- ----------------------------------------------------------------
  ...
  -- application_review consent for Arjun (grantee = employer_b)
  ...
  -- Synthetic readiness result (immutability trigger is UPDATE/DELETE only)
  ...
  -- Application snapshot (unfinalized first)
  ...
  -- Finalize snapshot: compute SHA-256 of canonical material and set finalized_at.
  -- protect_finalized_snapshot trigger blocks UPDATE only when old.finalized_at IS NOT NULL.
  if not exists (
  ...
  -- ----------------------------------------------------------------
  -- APPLICATION EVENT SEQUENCE (idempotent: each event only appended
  -- when current_application_stage matches the expected from_stage)
  --
  -- Valid lifecycle: preparing→applied→under_review→offered→accepted→active→completed→outcome_recorded
  --
  -- Fix 1: preparing→applied includes application_snapshot_id = v_snap_hist
  -- Fix 2: no direct applied→active (corrected to applied→under_review→offered→accepted→active)
  -- Fix 3: unique constraint application_events_one_submission enforces only one 'applied' event
  -- ----------------------------------------------------------------
  v_current_stage := sih26044.current_application_stage(v_app_hist);

  -- preparing → applied (with snapshot ID)
  if v_current_stage = 'preparing' then
    insert into sih26044.application_events (
      application_id, from_stage, to_stage, event_kind,
      application_snapshot_id, actor_id, note, occurred_at
    ) values (
      v_app_hist, 'preparing', 'applied', 'stage_transition',
      v_snap_hist,  -- Fix 1: required snapshot binding
      v_cand_1, 'Submitted application for Live Industry Data Challenge.', '2026-07-01T07:00:00Z'::timestamptz
    );
    v_current_stage := 'applied';
  end if;

  -- applied → under_review (Fix 2: was incorrectly applied→active)
  if v_current_stage = 'applied' then
    insert into sih26044.application_events (
      application_id, from_stage, to_stage, event_kind, actor_id, note, occurred_at
    ) values (
      v_app_hist, 'applied', 'under_review', 'stage_transition',
      v_actor_recruiter_b, 'Application reviewed; team shortlisted for project cohort.', '2026-07-02T09:00:00Z'::timestamptz
    );
  ...
  if v_current_stage = 'under_review' then
    insert into sih26044.application_events (
      ...
      v_app_hist, 'under_review', 'offered', 'stage_transition',
      v_actor_recruiter_b, 'Arjun Nair offered a place in the live project cohort.', '2026-07-02T10:00:00Z'::timestamptz
    );
    v_current_stage := 'offered';
  end if;

  -- offered → accepted (applicant accepts)
  if v_current_stage = 'offered' then
    insert into sih26044.application_events (
      application_id, from_stage, to_stage, event_kind, actor_id, note, occurred_at
    ) values (
      v_app_hist, 'offered', 'accepted', 'stage_transition',
      v_cand_1, 'Accepted the place in the Live Industry Data Challenge cohort.', '2026-07-03T08:00:00Z'::timestamptz
    );
  ...
  if v_current_stage = 'accepted' then
    insert into sih26044.application_events (
      ...
      v_app_hist, 'accepted', 'active', 'stage_transition',
      v_actor_recruiter_b, 'Live project started. Arjun Nair actively working on health analytics challenge.', '2026-07-05T09:00:00Z'::timestamptz
    );
  ...
  if v_current_stage = 'active' then
    insert into sih26044.application_events (
      ...
      v_actor_recruiter_b, 'Live project concluded. Top team presented strong findings on seasonal appointment patterns.', '2026-07-31T17:00:00Z'::timestamptz
    );
  ...
  -- Outcome event (validate_application_linked_outcome now satisfied:
  --   application_id IS NOT NULL, stage = outcome_recorded, subject/org/opp match)
  ...
  -- ----------------------------------------------------------------
  -- INSTITUTION INTERVENTIONS (all draft — Fix D: no events seeded)
  -- ----------------------------------------------------------------
  ...
  values (v_intv_evidence_clinic, v_org_institution, 'evidence_clinic', 'Portfolio Evidence Clinic – Data Visualisation', 'Aggregate skills intelligence shows only 29% of final-year students have recorded evidence for data visualisation.', 'Run three structured portfolio clinics in September 2026 where students can submit, review, and strengthen their data visualisation evidence with faculty guidance.', 'Final-year students in data/analytics programmes with low or absent data visualisation evidence', v_actor_inst_admin, v_actor_inst_admin, 'draft', 'sih26044-skills-intel-v1', '2026-09-01'::timestamptz, '2026-06-01'::timestamptz, '2026-08-31'::timestamptz, 'evidence_coverage_rate', '{"skillCategory": "data_visualization"}'::jsonb, 23, 80, 80, 'descriptive', v_fingerprint, '2026-09-01'::timestamptz)
  on conflict (id) do nothing;
  ...
  insert into sih26044.institution_interventions (id, organization_id, kind, title, rationale, action_description, intended_population_description, owner_actor_id, created_by_actor_id, initial_status, source_methodology_version, source_generated_at, source_window_from, source_window_to, source_metric, source_dimensions, source_value, source_denominator, source_cohort_size, source_interpretation, source_point_fingerprint, created_at)
  values (v_intv_sql_sprint, v_org_institution, 'training_support', 'SQL Practice Sprint – Analytics Readiness', 'Only 39% of students have recorded SQL evidence.', 'Host a 2-week SQL practice sprint with daily exercises, peer review, and a final assessed scenario.', 'Pre-final-year students without intermediate SQL evidence in their passport', v_actor_inst_admin, v_actor_inst_admin, 'draft', 'sih26044-skills-intel-v1', '2026-09-01'::timestamptz, '2026-06-01'::timestamptz, '2026-08-31'::timestamptz, 'evidence_coverage_rate', '{"skillCategory": "sql"}'::jsonb, 31, 80, 80, 'descriptive', v_fingerprint, '2026-09-02'::timestamptz)
  on conflict (id) do nothing;
  ...
  insert into sih26044.institution_interventions (id, organization_id, kind, title, rationale, action_description, intended_population_description, owner_actor_id, created_by_actor_id, initial_status, source_methodology_version, source_generated_at, source_window_from, source_window_to, source_metric, source_dimensions, source_value, source_denominator, source_cohort_size, source_interpretation, source_point_fingerprint, created_at)
  values (v_intv_research_workshop, v_org_institution, 'project_clinic', 'Research Documentation Workshop', 'Only 23% of students have recorded evidence for research methodology.', 'Conduct a half-day workshop on research documentation best practices.', 'Students applying for research-adjacent internships without research documentation evidence', v_actor_inst_admin, v_actor_inst_admin, 'draft', 'sih26044-skills-intel-v1', '2026-09-01'::timestamptz, '2026-06-01'::timestamptz, '2026-08-31'::timestamptz, 'evidence_coverage_rate', '{"skillCategory": "research_documentation"}'::jsonb, 18, 80, 80, 'descriptive', v_fingerprint, '2026-09-03'::timestamptz)
  on conflict (id) do nothing;
  ...
  insert into sih26044.institution_interventions (id, organization_id, kind, title, rationale, action_description, intended_population_description, owner_actor_id, created_by_actor_id, initial_status, source_methodology_version, source_generated_at, source_window_from, source_window_to, source_metric, source_dimensions, source_value, source_denominator, source_cohort_size, source_interpretation, source_point_fingerprint, created_at)
  values (v_intv_mentoring_cohort, v_org_institution, 'mentoring_cohort', 'Faculty–Industry Mentoring Cohort – Data Analytics Track', 'Placement readiness for final-year data analytics students is 65%.', 'Launch the Data Analytics Mentoring Cohort pairing 20 students with 10 industry mentors for 8 weeks.', 'Final-year students in data analytics track with placement readiness below NEAR_READY threshold', v_actor_inst_admin, v_actor_inst_admin, 'draft', 'sih26044-skills-intel-v1', '2026-09-01'::timestamptz, '2026-06-01'::timestamptz, '2026-08-31'::timestamptz, 'placement_readiness_rate', '{"programmeYear": "final", "track": "data_analytics"}'::jsonb, 42, 65, 65, 'descriptive', v_fingerprint, '2026-09-04'::timestamptz)
  on conflict (id) do nothing;
  ...
  select count(*) into v_cnt from sih26044.application_events where application_id = v_app_hist;
  ...
  select count(*) into v_cnt from sih26044.verification_requests where id = v_vreq_data_viz and status = 'requested';
  ...
  v_result := jsonb_set(v_result, '{message}', '"Phase-2B complete: legitimate historical outcome loop, corrected application lifecycle"'::jsonb);
  return v_result;
end;
$$;
...
comment on function sih26044.seed_controlled_demo_ecosystem_phase2b is
  'PR #89 repair: application_snapshot_id on applied event, corrected lifecycle preparing→applied→under_review→offered→accepted→active→completed→outcome_recorded, idempotent stage guards. All PR #87 fixes preserved. SECURITY DEFINER, service_role only.';
