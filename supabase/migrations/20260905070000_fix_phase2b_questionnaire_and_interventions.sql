-- Repair phase-2B seed function: three production-discovered defects fixed.
--
-- Fix A (already in repo via PR #86): invalid UUID hex chars f044q/f044qa → f0441001/f0441002
--
-- Fix B: questionnaire_questions conflict target
--   Wrong:  ON CONFLICT (id, questionnaire_version_id)        -- no such constraint
--   Right:  ON CONFLICT (questionnaire_version_id, ordinal)   -- the real UNIQUE key
--
-- Fix C: opportunity_questionnaire_assignments must include questionnaire_version_id
--   The table has FK: (questionnaire_version_id, questionnaire_id)
--                  -> questionnaire_versions(id, questionnaire_id)
--   Omitting questionnaire_version_id causes NOT NULL violation.
--
-- Fix D: institution_intervention_events must NOT be inserted by the bulk
--   SECURITY DEFINER seed. enforce_institution_intervention_event_sequence()
--   calls current_institution_intervention_status() → has_active_organization_role()
--   → current_actor_id() which returns NULL outside an authenticated session.
--   All 4 interventions are left at initial_status='draft'.
--   Lifecycle transitions are performed via BrowserOS Institution Admin walkthrough.
--
-- Event insert classification:
--   collaboration_engagement_events  → SAFE (direct INSERT bypasses RPC; no auth trigger on INSERT)
--   verification_events              → SAFE (immutability trigger fires only on UPDATE/DELETE)
--   outcome_events                   → SAFE (immutability trigger fires only on UPDATE/DELETE)
--   institution_intervention_events  → MUST be performed via authenticated role workflow
...
declare
  ...
  -- Institution interventions (NO lifecycle events seeded — performed via BrowserOS)
  ...
  -- Resolve authoritative flagship version (v1 or v2 depending on phase-2A repair path)
...
begin
  ...
  -- ----------------------------------------------------------------
  -- OPPORTUNITIES (insert as draft, requirements while draft, then publish)
  -- ----------------------------------------------------------------
  ...
  insert into sih26044.opportunity_versions (
    ...
    (v_ver_jr_analyst, v_opp_jr_analyst, 1, 'draft', 'Junior Data Analyst Intern',
     'Work with Meridian Analytics Labs to build and maintain data pipelines, perform exploratory analysis, and generate business insights from large structured datasets. Duration: 6 months | Hybrid, Bangalore | ₹18,000/month',
    ...
    (v_ver_prod_analytics, v_opp_prod_analytics, 1, 'draft', 'Product Analytics Intern',
     'Join the product team at Meridian Analytics Labs to measure feature adoption, diagnose funnel drop-offs, and translate data into actionable product decisions. Duration: 3 months | Remote | ₹15,000/month',
    ...
    (v_ver_research_asst, v_opp_research_asst, 1, 'draft', 'Research Data Assistant',
     'Support Pravaah Health Systems research team with data collection, cleaning, and documentation for ongoing AYUSH clinical studies. Duration: 4 months | On-site, Pune | ₹12,000/month',
    ...
    (v_ver_bi_intern, v_opp_bi_intern, 1, 'draft', 'Business Intelligence Intern',
     'Build and maintain BI dashboards in Tableau/Power BI for Meridian Analytics Labs. Own data model documentation and train end users on self-serve analytics. Duration: 6 months | Hybrid | ₹20,000/month',
    ...
    (v_ver_ops_analytics, v_opp_ops_analytics, 1, 'draft', 'Operations Analytics Intern',
     'Analyse supply chain and logistics data for Pravaah Health Systems. Build KPI reports and present recommendations to operations leadership. Duration: 3 months | Hybrid, Mumbai | ₹14,000/month',
    ...
    (v_ver_backend_dev, v_opp_backend_dev, 1, 'draft', 'Backend Developer Intern',
     'Build REST APIs and microservices for Meridian Analytics Labs data platform. Work with Python/FastAPI, PostgreSQL, and cloud infrastructure. Duration: 6 months | Remote | ₹22,000/month',
    ...
    (v_ver_ux_research, v_opp_ux_research, 1, 'draft', 'UX Research Assistant',
     'Support user research activities at Pravaah Health Systems: conduct user interviews, synthesise findings, and present insights to product and design teams. Duration: 3 months | Hybrid, Bangalore | ₹12,000/month',
  ...
  -- Requirements while all versions are still DRAFT
  ...
  -- Publish all versions (UPDATE draft → published after children inserted)
  ...
  select count(*) into v_cnt from sih26044.opportunity_requirements where opportunity_version_id in (
    v_ver_jr_analyst, v_ver_prod_analytics, v_ver_research_asst, v_ver_bi_intern,
    v_ver_ops_analytics, v_ver_backend_dev, v_ver_ux_research, v_ver_live_project,
    v_ver_mentoring, v_ver_workshop, v_ver_faculty_fdp, v_ver_collab_research, v_ver_guest_lecture
  );
  ...
  -- ----------------------------------------------------------------
  -- QUESTIONNAIRES: draft parent + draft version + questions (while draft) + publish
  -- ----------------------------------------------------------------
  ...
  -- FIX B: Questions inserted while versions are DRAFT.
  -- Conflict target: (questionnaire_version_id, ordinal) — the actual UNIQUE constraint.
  insert into sih26044.questionnaire_questions (
    ...
    -- Data Analyst (4 questions) — IDs use f0441001-... (all-hex, fixed in PR #86)
  ...
  -- FIX B: correct conflict target — the actual UNIQUE key on questionnaire_questions
  on conflict (questionnaire_version_id, ordinal) do nothing;

  -- Publish questionnaire versions (UPDATE draft → published after questions inserted)
  ...
  -- FIX C: questionnaire-opportunity assignments must include questionnaire_version_id
  -- FK: (questionnaire_version_id, questionnaire_id) -> questionnaire_versions(id, questionnaire_id)
  ...
  -- ----------------------------------------------------------------
  -- BACKGROUND APPLICATIONS (saved/preparing — no snapshot needed)
  -- ----------------------------------------------------------------
  ...
  -- ----------------------------------------------------------------
  -- CONSENT GRANTS + VERIFICATION REQUESTS + EVENTS
  -- verification_events: SAFE for direct insert (immutability trigger is UPDATE/DELETE only)
  -- ----------------------------------------------------------------
  ...
  -- ----------------------------------------------------------------
  -- COLLABORATIONS
  -- collaboration_engagement_events: SAFE for direct INSERT
  -- (revoke insert from authenticated only; SECURITY DEFINER bypasses that;
  --  no actor-authority trigger on INSERT — immutability fires only on UPDATE/DELETE)
  -- ----------------------------------------------------------------
  ...
  -- Collaboration engagement events (sequence 1 auto-created by trigger on INSERT)
  ...
  -- FIX D: NO institution_intervention_events seeded here.
  -- enforce_institution_intervention_event_sequence() calls
  -- current_institution_intervention_status() → has_active_organization_role()
  -- → current_actor_id() which returns NULL outside an authenticated session.
  -- All 4 interventions are left at initial_status = 'draft'.
  -- Lifecycle transitions (draft→approved, approved→active) are performed
  -- during the BrowserOS Institution Admin walkthrough.
  -- ----------------------------------------------------------------
  ...
  insert into sih26044.institution_interventions (
    ...
    (v_intv_research_workshop, v_org_institution, 'project_clinic',
     ...
     'Conduct a half-day workshop on research documentation best practices: literature review structure, data collection protocols, reproducibility standards.',
  ...
  -- ----------------------------------------------------------------
  -- OUTCOME LOOP
  -- outcome_events: SAFE for direct insert (immutability trigger is UPDATE/DELETE only)
  -- ----------------------------------------------------------------
...
comment on function sih26044.seed_controlled_demo_ecosystem_phase2b is
  'Fully repaired Phase-2B seed (PR #87). All UUID literals valid hex. Questionnaire conflict on (version_id, ordinal). Assignments include questionnaire_version_id. No institution_intervention_events seeded — lifecycle performed via BrowserOS Institution Admin walkthrough.';
